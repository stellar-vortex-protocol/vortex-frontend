...
describe("MyIntentsPage", () => {
  ...
  it("shows a connect prompt with ConnectWalletButton when wallet is not connected", () => {
    ...
    expect(
      screen.getAllByRole("button", { name: /Connect Freighter/i })[0],
    ).toBeInTheDocument();
  ...
  it("renders the page heading", () => {
    ...
    render(<MyIntentsPage />);
    expect(
      screen.getByRole("heading", { name: "My Intents" }),
    ).toBeInTheDocument();
  });
  ...
  it("renders loading skeleton with aria-hidden and status announcement", () => {
    ...
    expect(screen.getByText("Loading your intents...")).toHaveAttribute(
      "role",
      "status",
    );
  });
  ...
  it("shows the empty state with a CTA when the wallet has no swaps", () => {
    ...
    render(<MyIntentsPage />);
    expect(
      screen.getByText(/haven't submitted any swaps/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("links each intent row to the intent detail page", () => {
    ...
    const detailLinks = screen
      .getAllByRole("link")
      .filter((link) => link.getAttribute("href")?.startsWith("/explore/"));
  ...
  it("shows filter-empty state (not the no-swaps CTA) when filters exclude all results", async () => {
    ...
    expect(
      screen.queryByRole("link", { name: /make your first swap/i }),
